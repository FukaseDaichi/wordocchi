...
このドキュメントは、ワードッチ親向け進行補助ツールのビジュアルデザインと UX を、実装可能な粒度で定義する。Tailwind CSS のユーティリティクラスだけで再現できることを前提に、デザイントークン、コンポーネント、レイアウト、状態遷移ごとの画面設計を確定する。
...
| キャラクター | 犬 / 猫 イラスト | `public/illustrations/` |

## イラストレーション方針

画像にあるような探偵犬 + メモ猫のキャラクターを画面のキーポイントに置く。
...
- 形式: SVG (`public/illustrations/*.svg`)
- スタイル: 線画 + 塗り、線色 `#3D2E1F`、彩度はやや低め
...
- タイトルの一文字ずつに別カラーを当てる。画像と同じく `rose → sun → leaf → sky → rose` の配色
- キャラクター SVG は装飾なので `aria-hidden`
...
- 0秒になったら音 (設定でオンの場合) と画面全体に薄い `rose` の overlay フラッシュ
...
| アセット | パス | サイズ | 形式 |
|---|---|---|---|
| 犬探偵キャラ | `public/illustrations/dog-detective.svg` | viewBox 200×200 | SVG |
| 猫メモキャラ | `public/illustrations/cat-notekeeper.svg` | viewBox 200×200 | SVG |
| 鍵イラスト (秘匿) | `public/illustrations/lock.svg` | viewBox 120×120 | SVG |
| 装飾 (星・肉球) | `public/illustrations/decor-*.svg` | 各 40×40 | SVG |
| PWA アイコン | `public/icons/icon-{192,512}.png`, `maskable-512.png` | 仕様通り | PNG |
| OG 画像 | `public/og-image.png` | 1200×630 | PNG |

イラストは線色 `#3D2E1F`、塗り色はパレットから選ぶ。
...
- イラストコンポーネント: `XxxIllustration` (例: `LockIllustration`)
...
- 効果音を入れる場合、`<button>` の `onPointerDown` で再生し、`prefers-reduced-motion` 時はミュート
...
- カスタム CSS フレームワーク / UI ライブラリ (Tailwind ユーティリティのみで完結)
