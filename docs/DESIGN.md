...
> 2026-05-27 改訂: Hallmark デザインスキル(`anti-AI-slop`)に基づく Hallmark redesign を適用。Hero に犬探偵 + 猫メモのマスコット SVG を実装、PhaseStepper にグラデーション進行サムラインを追加、FooterBar の「新しく/はじめる」2行表記 (Hallmark Gate 59 違反) を「はじめ直す」1行に統一、TimerDisplay を線形バーから SVG 円形リングに刷新、PhasePanel に上部フェイズカラー帯、CriterionPicker は A/B 色分け、PromptWords はカードスタック (rose/sun/sky 三色、わずかな回転と staggered shadow) に変更。詳細は本ドキュメントの各セクションを参照。
...
| `surface-secret` | `rose-100` + `border-dashed border-rose-400` (親だけが見るヒミツのキジュン表示) |
...
| `shadow-pop` | `0 4px 0 rgba(110,80,40,0.08), 0 2px 6px rgba(110,80,40,0.10)` | 立体感を出すワードカードやステッパー active ノード |
...
| モーダルオーバーレイ | 200ms | `ease-out` | opacity 0 ↔ 1 (開閉両方向) |
| モーダル本体 (mobile) | 300ms | `[0.22, 1, 0.36, 1]` | y: 100% ↔ 0 (Bottom Sheet)、開閉どちらもアニメーション |
| モーダル本体 (≥sm) | 300ms | `[0.22, 1, 0.36, 1]` | scale 0.96 ↔ 1 + y: 12px ↔ 0 + fade |
...
**2026-05-27 更新:** ヘッダーには小さなカラフル「ワードッチ」ワードマーク (`text-base`) を中央に配置し、メニュー (`SettingsIcon`) と ルール (`HelpCircleIcon`) を `h-11 w-11` の `IconButton` に統一。Hero と被るカラー表示は色のサイズ差で区別する(ヘッダーは `text-base`、Hero は `text-[2.6rem]`/`sm:text-6xl`)。ヘッダー高は `h-12` (`px-3 py-2`) に切り詰め、スクロール時の縦占有を最小化。

### PhaseStepper (パンくず兼ステッパー)

**2026-05-27 更新:** ノードの上に「Phase N / 6」のラベル + グラデーション (`leaf-500 → sun-400 → rose-400`) のプログレスサムラインを `h-1` で追加。 ノードのサイズは `h-7 w-7`、ラベルは `text-[11px]` にスリム化して、進行ステッパー全体の高さを抑える(従来 `h-14` → 実質 `pt-2 pb-2.5 + 1.75rem`)。connector は `h-[3px]` の丸ピル。`active` ノードは `shadow-pop` で立体感を加える。
...
#### RestartButton (常設「はじめ直す」)

**2026-05-27 更新:** ボタンラベルを 2行 (`新しく / はじめる`) から 1行 (`はじめ直す`) に変更。Hallmark Slop-test Gate 59 (no two-line clickable text) に準拠。アイコンとラベルは水平並びの `flex h-14 items-center gap-1.5 px-3.5` に。aria-label は「新しくはじめる」のまま維持し、視覚と意味の橋を保つ。

```tsx
<button
  className="flex h-14 items-center gap-1.5 px-3.5 rounded-2xl bg-cream-100 text-ink-900 shadow-card hover:bg-cream-200 active:scale-[0.97] transition"
  aria-label="新しくはじめる"
>
  <RotateCcwIcon className="w-5 h-5 text-rose-500" />
  <span className="text-sm font-rounded font-bold whitespace-nowrap">はじめ直す</span>
</button>
```
...
**2026-05-27 実装:** マスコットは `src/components/illustrations/Mascots.tsx` で `DogDetective`(黄色頭+虫眼鏡)と `CatNotekeeper`(青みグレー耳+メモ帳) を SVG で配置。`grid-cols-[auto_1fr_auto]` で左右にキャラ、中央にワードマーク。背景は `from-sun-400/20 via-rose-100/40 to-sky-100/30 blur-2xl` のソフトグロー。マスコットは `motion-safe:animate-[float-soft_5s_ease-in-out_infinite]` で上下にやさしく揺れ、左右で `--rot` を反転させた傾き(`-6deg` / `6deg`)を持たせる。

```tsx
<section className="relative pb-3 pt-2" aria-label="ワードッチ">
  <div className="pointer-events-none absolute inset-x-0 top-4 mx-auto h-40 max-w-md rounded-full bg-gradient-to-br from-sun-400/20 via-rose-100/40 to-sky-100/30 blur-2xl" />
  <div className="relative grid grid-cols-[auto_1fr_auto] items-center gap-2 px-1">
    <DogDetective className="h-20 w-20 sm:h-24 sm:w-24 motion-safe:animate-[float-soft_5s_ease-in-out_infinite]" style={{ ['--rot']: '-6deg' }} />
    <div className="text-center">
      <p className="inline-flex items-center gap-1 rounded-full bg-cream-100 px-2.5 py-1 text-[10px] font-rounded font-extrabold uppercase tracking-[0.18em] text-ink-600 shadow-card">
        <SparklesIcon className="h-3 w-3 text-sun-400" /> 親のための進行サポート
      </p>
      <h2 className="mt-2 font-rounded text-[2.6rem] sm:text-6xl font-black tracking-tight">
        <span className="text-rose-500">ワ</span>...
      </h2>
      <p className="mt-1 text-xs sm:text-sm font-bold text-ink-600">あたたかい絵本のような進行カード</p>
    </div>
    <CatNotekeeper className="h-20 w-20 sm:h-24 sm:w-24 motion-safe:animate-[float-soft_5s_ease-in-out_infinite_0.6s]" style={{ ['--rot']: '6deg' }} />
  </div>
</section>
```

このセクションは `setup` フェイズだけ大きく表示し、他のフェイズでは「親のための進行サポート」のコンパクトピル (1行、キャラなし) に切り替える。

### PhasePanel

**2026-05-27 更新:** `Card` コンポーネントに `phaseTone` プロパティ (`"sun" | "rose" | "sky" | "leaf"`) を追加。フェイズに応じて上部に `h-1.5` のグラデーション帯を表示し、視覚で即座にフェイズを識別できるようにする。ヘッダー部はフラグアイコン + 「現在のフェイズ」キャプション(`tracking-[0.18em] uppercase`)+ 右側に大文字バッジ。バッジ表記の `！` を削除して(視覚ノイズ削減)、トーンを落ち着かせる。
...
<Card phaseTone={meta.tone} className="pt-6">
  <div className="flex items-center justify-between gap-3">
    <div className="flex items-center gap-2 text-leaf-600">
      ...
      <span className="text-[11px] font-rounded font-extrabold uppercase tracking-[0.18em]">現在のフェイズ</span>
    </div>
    <span className={cn(badgeBaseClass, badgeClass(meta.tone))}>{meta.badge}</span>
  </div>
  <h2 className="mt-3 font-rounded text-2xl sm:text-3xl font-extrabold text-ink-900">{heading}</h2>
  <p className="mt-2 text-sm sm:text-base text-ink-600 leading-relaxed">{description}</p>
  <div className="mt-5">{children /* フェイズ固有の中身 */}</div>
</Card>
```

#### phaseTone のグラデーション帯

| phaseTone | グラデーション (`bg-gradient-to-r`) |
|---|---|
| `sun` | `from-sun-400 via-rose-400 to-leaf-500` |
| `rose` | `from-rose-500 via-rose-400 to-sun-400` |
| `sky` | `from-sky-500 via-sky-500/70 to-leaf-500` |
| `leaf` | `from-leaf-500 via-sun-400 to-rose-400` |
...
このツールは親しか見ないため、選んだヒミツのキジュンは Main 領域の**一番上に常時表示**する。覗き見が心配なときだけ、右端の目アイコンで表示・非表示を切り替える。**デフォルトは表示**。
...
>
  <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-rose-500 text-white shadow-card">
    <LockKeyholeIcon className="h-5 w-5" aria-hidden />
  </span>
  <div className="min-w-0 flex-1">
    <p className="text-[10px] font-rounded font-extrabold uppercase tracking-[0.16em] text-rose-500">
      ヒミツのキジュン・親だけ
    </p>
    {/* 表示中はキジュン本文。非表示中は ●●●●●● のマスク (aria-hidden) */}
  </div>
  <button
    type="button"
    aria-pressed={isVisible}
    aria-label={isVisible ? "キジュンを隠す" : "キジュンを表示する"}
    className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-cream-50 text-rose-500 shadow-card active:scale-95"
  >
    {/* 表示中は EyeOffIcon (隠す)、非表示中は EyeIcon (表示する) */}
  </button>
</section>
```

- `secretCriterion` が決まる `wordPrompt` 以降に表示し、`reveal` / `done` では `RevealPanel` が全員へ公開するため出さない。
- 表示・非表示はその場限りの UI 状態 (`useState`)。保存しない。長押しモーダルや `Collapsible` は使わない。
...
**2026-05-27 改修:** 線形のプログレスバーから SVG 円形プログレスリングに刷新。リング自体は `viewBox="0 0 200 200"`、`r=88`、`stroke=12`、開始位置を 12時方向(`-rotate-90`)に固定し、`strokeDasharray` + `strokeDashoffset` で残量を表現。リング色は `isUrgent`(残り60秒以下)で `leaf-500 → rose-500` に変化。中央に `text-6xl sm:text-7xl font-black tabular-nums` で時間表示。背景には `bg-cream-100/60` の控えめなドーナツ穴。urgent 時はリング下に `bg-rose-100` のピル `ラストスパート` を表示。

```tsx
<div className="relative mt-3 flex h-52 w-52 sm:h-60 sm:w-60 items-center justify-center">
  <svg viewBox="0 0 200 200" className="absolute inset-0 h-full w-full -rotate-90">
    <circle cx="100" cy="100" r={88} stroke="#F0E2C8" strokeWidth={12} fill="none" />
    <circle
      cx="100" cy="100" r={88}
  ...
  <div className="absolute inset-6 rounded-full bg-cream-100/60" />
  <span className={cn("relative font-rounded text-6xl sm:text-7xl font-black tabular-nums", isUrgent && "animate-pulse text-rose-500")} aria-live="polite">
...
**2026-05-27 改訂:** タイマー以外の項目は初期スコープから外し、「既定のタイマー」のみを残す。`SegmentedControl` の平坦なトグルから、視覚的に長さを示せる円リング + 配色カードへ刷新。「保存データを初期化する」は誤操作リスクと初期スコープ簡素化の観点から廃止。

- 既定タイマー (`SettingsContent`: 3 / 5 / 7 / 10 分の 2×2 カード)
  - 各カードに `DurationRing` (10分基準のリング充填率) + 大きな分数 + 短いサブタイトル ("サクッと一勝負" など)
  - 配色は `sun` (3分) / `leaf` (5分・推奨) / `sky` (7分) / `rose` (10分) で覚えやすく
  - 選択中は配色の塗りつぶし + 右上に check ピル、未選択は `cream-100` ベース + hover で淡いグロー

### キジュンの表示・非表示

専用モーダルは設けない。選んだキジュンは `SecretCriterionBanner` が Main の一番上に常時表示し、覗き見が心配なときだけ右端の目アイコンでその場で隠す。

- 切替: バナー右端のボタンを**クリック / タップ**で表示 ↔ 非表示。**デフォルトは表示**
- 非表示中は本文を `●●●●●●` マスクに置き換え (`aria-hidden`)、ボタンは `EyeIcon` (表示する)
- 表示中はボタンが `EyeOffIcon` (隠す)。`aria-pressed={isVisible}` で状態を伝える
- 状態はその場限りの `useState`。保存も長押しもしない
...
| パネル内 | `WordCard` ×3 |
| SecretCriterionBanner | 選んだキジュンを一番上に常時表示 (目アイコンで表示・非表示) |
| FooterBar 左 | 「↻ 新しくはじめる」 |
...
- **キジュン表示切替**: 目アイコンのボタンに `aria-pressed={isVisible}` と「キジュンを隠す / 表示する」の `aria-label` を付け、SR でも状態を伝える
...
**2026-05-27 改訂:** 実装に合わせて構成を更新。マスコット SVG (`src/components/illustrations/Mascots.tsx`) を新設。Radix UI と Framer Motion は依存に含めず、純粋な CSS keyframes と `motion-safe:`/`motion-reduce:` ユーティリティで実装する方針(static export + GitHub Pages の軽量配信を優先)。

```text
src/
  app/
    layout.tsx                       # 永続レイアウト + フォント読み込み
    page.tsx                         # WordocchiApp を呼ぶだけの薄いシェル
    globals.css                      # base + keyframes (pop, slide-up-fade, sheet-up, overlay-fade, float-soft, ring-pulse)
    manifest.ts
  components/
    illustrations/
      Mascots.tsx                    # ★ 新規 (2026-05-27): DogDetective / CatNotekeeper / WordmarkSparkle SVG
    layout/
      AppHeader.tsx                  # 中央カラフルワードマーク + 設定/ルール icon
      PhaseStepper.tsx               # ★ 更新: 進行サムライン + Phase N/6 ラベル + h-7 ノード
      FooterBar.tsx                  # ★ 更新: 「はじめ直す」1行表記 (Gate 59 準拠), 主CTA に光沢ハイライト
      AppShell.tsx                   # max-w-screen-sm/lg:max-w-5xl の中央寄せ
    ui/
      Button.tsx                     # primary/secondary/ghost/danger/secret バリアント
      Card.tsx                       # ★ 更新: phaseTone (上部グラデーション帯) 対応
      ConfirmDialog.tsx              # Modal + AlertTriangleIcon
      IconButton.tsx                 # ★ 更新: showLabel オプション (デフォルトは icon のみ)
      Modal.tsx                      # bottom sheet (mobile) / center dialog (≥sm) 切替
      SegmentedControl.tsx           # タイマー時間選択用
      TimerDisplay.tsx               # ★ 改修: SVG 円形プログレスリング
    pwa/
  ...
  features/
    round/
      ...
      round-types.ts
    rules/
      RulesDialog.tsx                # ルールモーダル
      RulesCarousel.tsx              # 自前カルーセル (Embla 同等のステップ送り)
      rules-copy.ts                  # 5ステップのコピー
    timer/
      components/
        CountdownControls.tsx        # 一時停止 / リセット / 終了 の3列
      timer-utils.ts
      use-countdown.ts
  lib/
    cn.ts                            # clsx ラッパー
    constants.ts                     # APP_NAME, タイマー定数
    base-path.ts                     # GitHub Pages basePath helper
    storage.ts                       # localStorage ラッパー
  data/
    sample-prompts.json              # 独自サンプルキジュン/ワード
    sample-prompts.ts
```
...
- [ ] 全画面 (reveal を除く) で `FooterBar` 左に「はじめ直す」が常設されている
- [ ] `setup` 以外で「はじめ直す」を押すと確認ダイアログが出る
- [ ] `timedInvestigation` の `CountdownControls` に「終了」ボタンがある
- [ ] 「終了」を押すと確認後に `finalGuide` へ進む
- [ ] ルールモーダルが Bottom Sheet (mobile) / Center Dialog (≥sm) で開く
...
### Hallmark Slop-test (2026-05-27 追加)

Hallmark redesign の post-emit チェック。すべて NO であること。

- [ ] **Gate 36** 320/375/414/768 px のいずれの幅でも横スクロール 0
- [ ] **Gate 59** 1つのクリック可能要素に2行以上のテキストがない (FooterBar の「はじめ直す」、ヘッダー IconButton、Stepper ノードすべて 1 行)
- [ ] **Gate 46-50** 主要文字色 / 背景色のコントラストが WCAG AA 以上
- [ ] **Gate 57** 偽のブラウザ chrome / 端末枠 / コードウィンドウ装飾を持たない
- [ ] **Gate 58** 色とフォントはトークン経由のみ(`tailwind.config.ts` 経由)で参照、生 hex / OKLCH のインライン記述なし
- [ ] **Gate 66** 左マージン番号ラベル + 大型見出しの「hanging header」を使っていない
