...
import type { Round, RoundAction, RoundPhase } from "@/features/round/round-types";
import { RulesContent } from "@/features/rules/RulesContent";
  ...
  return (
    <>
      ...
      <AppShell>
        ...
        <main className="grid gap-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.8fr)] lg:items-start">
          <section
            className="rounded-3xl bg-cream-100 p-5 shadow-card sm:p-6"
          ...
          >
            ...
            <h1
              ...
              tabIndex={-1}
              className={cn(
                "mt-3 font-rounded text-2xl font-extrabold leading-snug text-ink-900 sm:text-3xl",
            ...
            <div className="mt-5">{renderPhaseBody(phase, round, dispatch, startRound)}</div>
          </section>
      ...
      <Modal
        ...
        onClose={closeRules}
        footer={
          <Button intent="primary" onClick={closeRules}>
            とじる
          </Button>
        }
      >
  ...
  switch (phase) {
    case "setup":
      return (
        <Button
          intent="primary"
          leadingIcon={<PlayIcon className="h-6 w-6" />}
          onClick={startRound}
        >
          新しくはじめる
        </Button>
    ...
    case "secretSelection":
      return round ? (
        <CriterionPicker
          ...
          onPick={(criterionId) => dispatch({ type: "round/chooseCriterion", criterionId })}
        />
      ) : null;
    ...
    case "wordPrompt":
      return round ? (
        <div className="space-y-5">
          <PromptWords words={round.promptWords} />
          <Button
            intent="primary"
            leadingIcon={<PlayIcon className="h-6 w-6" />}
            onClick={() =>
              dispatch({ type: "round/startInvestigation", now: new Date().toISOString() })
            }
          >
            タイマーを開始する
          </Button>
      ...
      return round ? (
        <div className="space-y-5">
          ...
          <CountdownControls
            ...
            remainingSeconds={round.timerRemainingSeconds}
            onStart={() => dispatch({ type: "timer/start", now: new Date().toISOString() })}
            onPause={() => dispatch({ type: "timer/pause", now: new Date().toISOString() })}
            onResume={() => dispatch({ type: "timer/resume", now: new Date().toISOString() })}
            onReset={() => dispatch({ type: "timer/reset" })}
          />
        </div>
      ) : null;
      ...
      return (
        <div className="space-y-5">
          <div className="rounded-2xl bg-rose-100 p-4">
            <p className="font-rounded text-lg font-bold text-rose-500">時間です</p>
            <p className="mt-2 text-sm leading-relaxed text-ink-600">
              調査フェイズを終えて、全員に最後のワードを考えてもらいましょう。
            </p>
          </div>
          <Button
            intent="primary"
            leadingIcon={<SparklesIcon className="h-6 w-6" />}
            onClick={() => dispatch({ type: "round/reveal", now: new Date().toISOString() })}
          >
            キジュンを公開する
          </Button>
        </div>
      ...
      return round ? (
        <div className="space-y-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Button
              intent="secondary"
              onClick={() => dispatch({ type: "round/finish", now: new Date().toISOString() })}
            >
              終わる
            </Button>
          </div>
...
function Hero() {
  ...
  return (
    <section className="relative px-4 pb-4 pt-2 text-center">
